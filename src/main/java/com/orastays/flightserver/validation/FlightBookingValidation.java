...
public class FlightBookingValidation extends AuthorizeUserValidation {
	...
	public void validateBookingDetails(FlightBookingModel flightBookingModel) throws FormExceptions {
		...
		if (Objects.isNull(flightBookingModel)) {
			exceptions.put(messageUtil.getBundle("booking.list.null.code"),
					new Exception(messageUtil.getBundle("booking.list.null.message")));
		} else {
			if (StringUtils.isBlank(flightBookingModel.getPricingId())) {
				exceptions.put(messageUtil.getBundle("pricing.id.null.code"),
						new Exception(messageUtil.getBundle("pricing.id.null.message")));
			}
			if (StringUtils.isBlank(flightBookingModel.getSuperPnr())) {
				exceptions.put(messageUtil.getBundle("superPnr.null.code"),
						new Exception(messageUtil.getBundle("superPnr.null.message")));
			}
			// ReviewJson
			if (Objects.isNull(flightBookingModel.getReviewJson())) {
				exceptions.put(messageUtil.getBundle("review.json.null.code"),
						new Exception(messageUtil.getBundle("review.json.null.message")));
			} else {
				// GlobalParams
				if (Objects.isNull(flightBookingModel.getReviewJson().getGlobalParamsModel())) {
					exceptions.put(messageUtil.getBundle("global.params.null.code"),
							new Exception(messageUtil.getBundle("global.params.null.message")));
				} else {
					...
					if (StringUtils.isBlank(flightBookingModel.getReviewJson().getGlobalParamsModel().getSearchId())) {
						exceptions.put(messageUtil.getBundle("search.id.null.code"),
								new Exception(messageUtil.getBundle("search.id.null.message")));
					}
					if (StringUtils.isBlank(flightBookingModel.getReviewJson().getGlobalParamsModel().getOrg())) {
						exceptions.put(messageUtil.getBundle("origin.null.code"),
								new Exception(messageUtil.getBundle("origin.null.message")));
					}
					if (StringUtils.isBlank(flightBookingModel.getReviewJson().getGlobalParamsModel().getDest())) {
						exceptions.put(messageUtil.getBundle("destination.null.code"),
								new Exception(messageUtil.getBundle("destination.null.message")));
					}
				}

				// travellerParams
				if (Objects.isNull(flightBookingModel.getReviewJson().getTravellerParamModels())) {
					exceptions.put(messageUtil.getBundle("traveller.params.null.code"),
							new Exception(messageUtil.getBundle("traveller.params.null.message")));
				} else {
					/*
